...
@section('content')
    ...
    <section class="container" style="margin-bottom: 40px;">
        <h2 class="best-seller-title" style="text-align:center; margin: 40px 0 24px 0;">Necklaces</h2>
        <div class="row" style="justify-content:center;" id="necklacesGrid"></div>
        <div class="text-center">
            <button id="showMoreNecklacesBtn" class="btn-custom" style="width:180px;margin-top:10px;">Show More</button>
        </div>
    </section>

    <script>
    document.addEventListener('DOMContentLoaded', function() {
        const totalNecklaces = 40;
        const productsPerClick = 4;
        let visibleCount = 8;

        function getNecklaceProduct(i) {
            return {
                img: `/images/necklace-${(i % 6) + 1}.png`, // replace with actual necklace image filenames
                name: `Necklace ${i + 1}`,
        ...
        const showMoreBtn = document.getElementById('showMoreNecklacesBtn');

        function renderNecklaces() {
            necklacesGrid.innerHTML = '';
            for (let i = 0; i < visibleCount && i < totalNecklaces; i++) {
                const p = getNecklaceProduct(i);
                necklacesGrid.innerHTML += `
            ...
            if (visibleCount >= totalNecklaces) {
