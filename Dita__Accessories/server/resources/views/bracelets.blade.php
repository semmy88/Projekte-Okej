...
@section('content')
    ...
    <section class="container" style="margin-bottom: 40px;">
        <h2 class="best-seller-title" style="text-align:center; margin: 40px 0 24px 0;">Bracelets</h2>
        <div class="row" style="justify-content:center;" id="braceletsGrid"></div>
        <div class="text-center">
            <button id="showMoreBraceletsBtn" class="btn-custom" style="width:180px;margin-top:10px;">Show More</button>
        </div>
    </section>

    <script>
    document.addEventListener('DOMContentLoaded', function() {
        const totalBracelets = 40;
        const productsPerClick = 4;
        let visibleCount = 8;

        function getBraceletProduct(i) {
            return {
                img: `/images/bracelet-${(i % 6) + 1}.png`, // replace with actual bracelet image filenames
                name: `Bracelet ${i + 1}`,
        ...
        const showMoreBtn = document.getElementById('showMoreBraceletsBtn');

        function renderBracelets() {
            braceletsGrid.innerHTML = '';
            for (let i = 0; i < visibleCount && i < totalBracelets; i++) {
                const p = getBraceletProduct(i);
                braceletsGrid.innerHTML += `
                    <div class="col-md-3 col-sm-6 mb-4 d-flex align-items-stretch product-card" data-index="${i}">
                        <div class="best-seller-card" style="width:100%;">
                            ...
                            <div class="best-seller-actions">
                                ...
                                <button class="btn-custom" style="width:40px;height:40px;display:flex;align-items:center;justify-content:center;padding:0;font-size:1.1rem;min-width:0;">
            ...
            if (visibleCount >= totalBracelets) {
