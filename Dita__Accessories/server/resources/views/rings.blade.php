...
@section('content')
    ...
    <section class="container" style="margin-bottom: 40px;">
        <h2 class="best-seller-title" style="text-align:center; margin: 40px 0 24px 0;">Rings</h2>
        <div class="row" style="justify-content:center;" id="ringsGrid"></div>
        <div class="text-center">
            <button id="showMoreRingsBtn" class="btn-custom" style="width:180px;margin-top:10px;">Show More</button>
        </div>
    </section>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function getRingProduct(i) {
            return {
                img: `/images/ring-${(i % 6) + 1}.png`, // replace with actual ring image filenames
                name: `Ring ${i + 1}`,
        ...
        const showMoreBtn = document.getElementById('showMoreRingsBtn');

        function renderRings() {
            ringsGrid.innerHTML = '';
            for (let i = 0; i < visibleCount && i < totalRings; i++) {
                const p = getRingProduct(i);
                ringsGrid.innerHTML += `
